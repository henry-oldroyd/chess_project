...
class Board_State():
    ...
    def __init__(self, next_to_go="W", pieces_matrix=STARTING_POSITIONS, pieces_matrix_frequency={}, check_encountered=None) -> None:
    ...
    def make_move(self, from_position_vector: Vector, movement_vector: Vector):
        ...
        # print(f"Setting piece {piece!r} at {from_position_vector!r} to have a last move property of {movement_vector!r}")
    ...
    # this function encodes MOST of the information that makes this object unique in a number.  
    # this doesn't include move history, this should make the cache more useful at the cost of making the program worse at avoiding 3 repeat stalemated
...
# this is done by selecting and then implementing a legal move at random up to so many moves
...
def random_board_state(moves: int):
    ...
    while True:
        try:
            # iteratively pick a move at random and implement it until the right number of move is reached
