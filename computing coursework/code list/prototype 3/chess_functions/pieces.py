...
class Piece():
    ...
    #     # instantiate the appropriate subclass of Piece based on the type field
    #     if piece_type == 'Pawn':
...
# restored form v2.4 still has issue with pawn moving foreword 2
...
class Pawn(Piece, Abstract_Piece_Interface):
    ...
    def __init__(self, color):
        ...
        # method defined here as it only used here
        # decided if the pawn is allowed to move foreword 2 based on square contents and last move

        # def can_move_foreword_2(to_square, middle_square):
        #     return (to_square is not None) and (middle_square is not None) and (self.last_move is None)
    ...
    def generate_movement_vectors(self, pieces_matrix, position_vector):
        ...
        for movement_vector, condition in self.movement_vector_and_condition:
            ...
            if movement_vector not in (Vector(0, 2), Vector(0, -2)):
                ...
                middle_square_vector = position_vector + movement_vector * (1/2)
        
                row, column = 7-middle_square_vector.j, middle_square_vector.i
                middle_square = pieces_matrix[row][column]

                if condition(to_square=to_square, middle_square=middle_square):
                    yield movement_vector
